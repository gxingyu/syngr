...
from models.model import SynergisticT5
from utils import *
from collator import Collator, SynergisticCollator
...
def train(args):
    ...
    collator = SynergisticCollator(args, tokenizer)
    
    if args.load_model_name is not None:
        model = SynergisticT5.from_pretrained(
            args.load_model_name, 
            config=config,

            alpha=getattr(args, 'alpha', 0.1), 
            mask_ratio=getattr(args, 'mask_ratio', 0.3),
            prompt_len=args.prompt_num
        )
    else:
        model = SynergisticT5(
            config,
