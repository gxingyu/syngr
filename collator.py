...
class SynergisticCollator(object):
    ...
    def __call__(self, batch):
        ...
        unimodal_texts = [d.get("unimodal_input_ids", "") for d in batch]
        has_unimodal_data = any(len(t) > 0 for t in unimodal_texts)
        ...
        if has_unimodal_data:
            unimodal_inputs = self.tokenizer(unimodal_texts,
                                             return_tensors="pt",
                                             padding="longest",
                                             max_length=self.tokenizer.model_max_length,
                                             truncation=True,
                                             return_attention_mask=True)
            inputs['unimodal_input_ids'] = unimodal_inputs['input_ids']
        else:

            inputs['unimodal_input_ids'] = None
