...
from datasets import EmbDataset, EmbDatasetAll, EmbDatasetOne, DualEmbDataset
...
for dataset in datasets:
    data = DualEmbDataset(config, dataset)
    ...
    for t,v,_ in tqdm(data_loader):
        t = t.to(device)
        v = v.to(device)
        indices, distances = model.get_indices(t, v, use_sk=False, text=(1 if config.content=='text' else 2))
